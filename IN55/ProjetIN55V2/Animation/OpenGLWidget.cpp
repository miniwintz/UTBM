...
void OpenGLWidget::conversionVecteursVersAngles() //transforme les coordonnees X,Y,Z en phi et theta
{
    ...
    //-La librairie <cmath> utilise les radians! C'est pour cela qu'il faut convertir à chaque fois les degres...
...
void OpenGLWidget::paintGL()
{
    ...
    m_targetcameraLibre = (*p_cameraLibre).getCibleCamera();

    m_positionCamera.setX(m_positioncameraLibre.x() + 1);
    m_positionCamera.setY(m_positioncameraLibre.y() + 1);
    m_positionCamera.setZ(m_positioncameraLibre.z() + 5);

    m_cibleCamera = m_targetcameraLibre;
    gluLookAt(m_positionCamera.x(), m_positionCamera.y(), m_positionCamera.z(), m_cibleCamera.x(), m_cibleCamera.y(), m_cibleCamera.z(), 0, 0, 1);


    p_basis->drawShape(); // on affiche le repere de la scene

    glEnable ( GL_TEXTURE_2D );
    drawSkybox(); // on dessine les murs et le plafond de la scene
